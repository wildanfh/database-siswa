...
<section>
  <div class="container mx-auto">
    <h2 class="text-slate-700">Tahun Masuk</h2>
    <div class="mb-4 mt-3" x-data="{ showSearch: false }">
      ...
      <form x-show="showSearch" id="search-form" action="{{ route('entryYears.index') }}" method="GET" class="flex h-8">
        <input type="text" name="search" placeholder="Search by tahun masuk" class="shadow appearance-none border rounded w-3/4 py-2 px-3 text-gray-700 text-sm focus:outline-none focus:shadow-outline">
    ...
    <div class="flex justify-end mb-4">
      <a href="{{ route('entryYears.create') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded text-sm">
        Create
      </a>
    </div>
    @if($entryYears->isEmpty())
        ...
        <div class="bg-gray-50 overflow-hidden sm:rounded-lg">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              @foreach ($entryYears as $i => $entryYear)
              <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <a href="{{ route('entryYears.show', $entryYear->id) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                  <a href="{{ route('entryYears.edit', $entryYear->id) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                  <form action="{{ route('entryYears.destroy', $entryYear->id) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
