...
<section>
  <div class="container mx-auto">
    <h2 class="text-slate-700">Units</h2>
    <div class="mb-4 mt-3" x-data="{ showSearch: false }">
      ...
      <form x-show="showSearch" id="search-form" action="{{ route('units.index') }}" method="GET" class="flex h-8">
        <input type="text" name="search" placeholder="Search by unit" class="shadow appearance-none border rounded w-3/4 py-2 px-3 text-gray-700 text-sm focus:outline-none focus:shadow-outline">
    ...
    <div class="flex justify-end mb-4">
      <a href="{{ route('units.create') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded text-sm">
        Create
      </a>
    </div>
    @if($units->isEmpty())
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <a href="{{ route('units.show', $unit->id) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                  <a href="{{ route('units.edit', $unit->id) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
